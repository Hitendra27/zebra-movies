...
export const searchByString = async ({
    ...
    try {
        ...
        const searchMovieResults: MovieWithGenreResponse[] = data.results.map((movie: any) => ({
            id: movie.id,
            title: movie.title,
            overview: movie.overview,
            poster_path: movie.poster_path,
...
export const getLatest = async () => {
    try {
        const url = `https://api.themoviedb.org/3/discover/movie?page=1&include_video=false&primary_release_date.gte=2023-08-07&primary_release_date.lte=2022-03-01?api_key=${process.env.API_KEY}`;
        const response = await fetch(url, options);
